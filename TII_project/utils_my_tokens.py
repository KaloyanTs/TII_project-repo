...
def readCorpus(fileName, tokens, token_list, unkToken):
    ...
    # return [ encode(line, tokens, token_list, unkToken) for line in open(fileName, encoding="utf-8") ]
...
def prepareData(sourceFileName, targetFileName, sourceDevFileName, targetDevFileName, startToken, endToken, unkToken, padToken, transToken):
    ...
    # word2ind = tokens
    # word2ind[startToken]=len(word2ind)
    # word2ind[endToken]=len(word2ind)
    # word2ind[unkToken]=len(word2ind)
    # word2ind[padToken]=len(word2ind)
    # word2ind[transToken]=len(word2ind)
    token_list = sorted(tokens.keys(), key=lambda x: len(x), reverse=True)
    sourceCorpus = readCorpus(sourceFileName, tokens, token_list, unkToken)
    targetCorpus = readCorpus(targetFileName, tokens, token_list, unkToken)
    ...
    sourceDev = readCorpus(sourceDevFileName, tokens,token_list, unkToken)
    targetDev = readCorpus(targetDevFileName, tokens, token_list, unkToken)
