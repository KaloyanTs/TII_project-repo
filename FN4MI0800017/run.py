...
if len(sys.argv) > 3 and sys.argv[1] == 'translate':
    ...
    for idx, s in enumerate(test):
        ...
        avg_time_per_iter = elapsed_time / (idx + 1)
        remaining_iters = len(test) - (idx + 1)
        est_remaining_time = avg_time_per_iter * remaining_iters

        est_hours, rem = divmod(est_remaining_time, 3600)
        est_minutes, est_seconds = divmod(rem, 60)
        if (idx + 1) % 20 == 0:
          print(f"\rEstimated time remaining: {int(est_hours):02}:{int(est_minutes):02}:{int(est_seconds):02}", end="")
